...
from decoder import converter, utils
import json

# supported JPEG markers
...
RST = tuple(
    bytes.fromhex(hex(marker)[2:]) for marker in range(0xFFD0, 0xFFD8)
)  # Restart markers
...
ColorComponent = namedtuple(
    "ColorComponent",
    "name order vertical_sampling horizontal_sampling quantization_table_id repeat shape",
)
...
class JpegDecoder:
    def __init__(self, file: Path) -> None:
        ...
        with open("markers.txt", "w") as self.markers:
            self.markers.write(f"{file}\n")
        ...
        # Assert if file is valid JPEG (starts with bytes Start of image xFF 'FFD8FF')
        assert self.raw_file.startswith(SOI + b"\xFF")
        ...
        self.file_header = (
            2  # Offset (in bytes, 0-index) from the beginning of the file
        )
        self.scan_finished = False
        self.scan_mode = None  # 'baseline_dct' or 'progressive_dct'
        self.image_width = 0
        self.image_height = 0
        self.color_components = {}  # Hold each component and its respective paramenters
        self.sample_shape = ()
        self.huffman_tables = {}
        ...
        """NOTE
        We are sequentially looking for markers in the file. 
        Found markers are handled by corresponding methods.
        Each marker begins with 0xFF. 
        If this byte is followed by a 0x00, then it is escaped.
        Segments with unsupported markers are skipped.
        """
        while not self.scan_finished:
            ...
            if current_byte == 0xFF:
                ...
                # If the two bytes form a marker
                if (my_marker != b"\xFF\x00") and (my_marker not in RST):

                    my_handler = self.handlers.get(my_marker)
                    my_size = (
                        converter.bytes_to_uint(
                            self.raw_file[self.file_header : self.file_header + 2]
                        )
                        - 2
                    )
                    self.file_header += 2
                    ...
                    if my_handler is not None:
                        my_data = self.raw_file[
                            self.file_header : self.file_header + my_size
                        ]
                        my_handler(my_data)
                    else:
    ...
    def start_of_frame(self, data: bytes) -> None:
        ...
        if mode == SOF0:
            ...
            with open("markers.txt", "a") as self.markers:
                self.markers.write(f"baseline\n")
            print("Scan mode: Sequential")
        elif mode == SOF2:
            self.scan_mode = "progressive_dct"
            with open("markers.txt", "a") as self.markers:
                self.markers.write(f"progressive\n")
            print("Scan mode: Progressive")
        else:
            raise UnsupportedJpeg(
                "Encoding mode not supported. Only 'Baseline DCT' and 'Progressive DCT' are supported."
            )
        ...
        if precision != 8:
            raise UnsupportedJpeg(
                "Unsupported color depth. Only 8-bit greyscale and 24-bit RGB are supported."
            )
        data_header += 1

        # Get image dimensions
        self.image_height = converter.bytes_to_uint(data[data_header : data_header + 2])
        data_header += 2
        """NOTE
        ...
        self.image_width = converter.bytes_to_uint(data[data_header : data_header + 2])
        data_header += 2
        with open("markers.txt", "a") as self.markers:
            self.markers.write(f"{self.image_width}x{self.image_height}\n")
        ...
        if components_amount not in (1, 3):
            if components_amount == 4:
                raise UnsupportedJpeg(
                    "CMYK color space is not supported. Only RGB and greyscale are supported."
                )
            else:
                raise UnsupportedJpeg(
                    "Unsupported color space. Only RGB and greyscale are supported."
                )
        data_header += 1

        if components_amount == 3:
            with open("markers.txt", "a") as self.markers:
                self.markers.write(f"YCbCr\n")
        ...
        try:
            for count, component in enumerate(components, start=1):
                ...
                my_component = ColorComponent(
                    ...
                    repeat=horizontal_sample
                    * vertical_sample,  # Amount of times the component repeats during decoding
                    shape=(
                        8 * horizontal_sample,
                        8 * vertical_sample,
        ...
        sample_width = max(
            component.shape[0] for component in self.color_components.values()
        )
        sample_height = max(
            component.shape[1] for component in self.color_components.values()
        )
        self.sample_shape = (sample_width, sample_height)

        # Display the samplings
        print(
            f"Horizontal sampling: {' x '.join(str(component.horizontal_sampling) for component in self.color_components.values())}"
        )
        print(
            f"Vertical sampling  : {' x '.join(str(component.vertical_sampling) for component in self.color_components.values())}"
        )
    ...
    def define_huffman_table(self, data: bytes) -> None:
        ...
        while data_header < data_size:
            ...
            codes_count = {
                bit_length: count
                for bit_length, count in zip(
                    range(1, 17), data[data_header : data_header + 16]
                )
            }
            data_header += 16
            ...
            with open("markers.txt", "a") as self.markers:
                self.markers.write(
                    f"Huffman table {table_destination & 0x0F} {'DC' if table_destination >> 4 == 0 else 'AC'}\n"
                )
            print(f"Parsed Huffman table - ", end="")
            print(
                f"ID: {table_destination & 0x0F} ({'DC' if table_destination >> 4 == 0 else 'AC'})"
            )
    ...
    def define_quantization_table(self, data: bytes) -> None:
        ...
        while data_header < data_size:
            ...
            qt_values = np.array(
                [value for value in data[data_header : data_header + 64]], dtype="int16"
            )
            try:
                quantization_table = utils.undo_zigzag(qt_values)
            except ValueError:
    ...
    def define_restart_interval(self, data: bytes) -> None:
        ...
        self.restart_interval = converter.bytes_to_uint(data[:2])
        self.file_header += 2
    ...
    def start_of_scan(self, data: bytes) -> None:
        ...
        for component in range(components_amount):
            ...
            my_huffman_tables.update(
                {component_id: HuffmanTable(dc=dc_table, ac=ac_table)}
            )
            my_color_components.update(
                {component_id: self.color_components[component_id]}
            )

        # Get spectral selection and successive approximation
        if self.scan_mode == "progressive_dct":
            spectral_selection_start = data[
                data_header
            ]  # Index of the first values of the data unit
            spectral_selection_end = data[
                data_header + 1
            ]  # Index of the last values of the data unit
            bit_position_high = (
                data[data_header + 2] >> 4
            )  # The position of the last bit sent in the previous scan
            bit_position_low = (
                data[data_header + 2] & 0x0F
        ...
        if self.image_height == 0:
            ...
            if dnl_index != -1:
                ...
                self.image_height = converter.bytes_to_uint(
                    self.raw_file[dnl_index + 4 : dnl_index + 6]
                )
            else:
        ...
        if self.image_array is None:
            ...
            self.image_array = np.zeros(
                shape=(self.array_width, self.array_height, self.array_depth),
                dtype="int16",
            )
    ...
    def baseline_dct_scan(
        self, huffman_tables_id: dict, my_color_components: dict
    ) -> None:
        ...
        with open("markers.txt", "a") as self.markers:
            self.markers.write(f"Scan {self.scan_count+1} of {self.scan_amount}\n")
            self.markers.write(
                f"Color components: {', '.join(component.name for component in my_color_components.values())}"
            )
            self.markers.write(f"MCU count: {self.mcu_count}")
        print(
            f"Color components: {', '.join(component.name for component in my_color_components.values())}"
        )
        ...
        def next_huffval() -> int:
            ...
            while huffman_value is None:
                ...
                if len(codeword) > 16:
                    raise CorruptedJpeg(
                        f"Failed to decode image ({current_mcu}/{self.mcu_count} MCUs decoded)."
                    )
        ...
        while current_mcu < self.mcu_count:
            ...
            for depth, (component_id, component) in enumerate(
                my_color_components.items()
            ):
                ...
                for block_count in range(repeat):
                    ...
                    dc_value = (
                        utils.bin_twos_complement(next_bits(huffman_value))
                        + previous_dc[depth]
                    )
                    ...
                    while index < 64:
                        ...
                        if ac_bit_length > 0:
                            ac_value = utils.bin_twos_complement(
                                next_bits(ac_bit_length)
                            )
                    ...
                    block = utils.undo_zigzag(block) * quantization_table
                ...
                self.image_array[
                    x : x + self.mcu_width, y : y + self.mcu_height, component.order
                ] = my_mcu

            # Go to the next MCU
            current_mcu += 1
            utils.print_progress(current_mcu, self.mcu_count)

            # Check for restart interval
            if (
                (self.restart_interval > 0)
                and (current_mcu % self.restart_interval == 0)
                and (current_mcu != self.mcu_count)
            ):
            ...
            """
        self.scan_count += 1
        utils.print_progress(current_mcu, self.mcu_count, done=True)

    def progressive_dct_scan(
        self,
        ...
        bit_position_low: int,
    ) -> None:

        # Whether to the scan contains DC or AC values
        if (spectral_selection_start == 0) and (spectral_selection_end == 0):
            values = "dc"
        elif (spectral_selection_start > 0) and (
            spectral_selection_end >= spectral_selection_start
        ):
            values = "ac"
        else:
            raise CorruptedJpeg(
                "Progressive JPEG images cannot contain both DC and AC values in the same scan."
            )
        """NOTE
        ...
        if bit_position_high == 0:
            ...
            raise CorruptedJpeg(
                "Progressive JPEG images cannot contain more than 1 bit for each value on a refining scan."
            )
        """NOTE
        ...
        print(
            f"Spectral selection: {spectral_selection_start}-{spectral_selection_end} ({values.upper()})"
        )
        print(
            f"Successive approximation: {bit_position_high}-{bit_position_low} ({'refining' if refining else 'first'} scan)"
        )
        print(f"MCU count: {self.mcu_count}")
        print(f"Decoding MCUs...")
        with open("markers.txt", "a") as self.markers:
            self.markers.write(f"\nScan {self.scan_count+1} of {self.scan_amount}\n")
            self.markers.write(
                f"Color components: {', '.join(component.name for component in my_color_components.values())}\n"
            )
            self.markers.write(
        ...
        # AC values scan
            ...
            def refine_ac() -> None:
                ...
                while to_refine:
                    ...
                    self.image_array[ref_x, ref_y, component.order] |= (
                        new_bit << bit_position_low
                    )
                    ref_index += 1
                ...
                while (
                    ...
                    if not refining and zero_run:  # First scan
                        ...
                        while zero_run > 0:  # Refining scan
                            xr, yr = utils.zagzig[index]
                            current_value = self.image_array[
                    ...
                    if ac_bits_length > 0:
                        ...
                        if refining:
                            while (
                                self.image_array[x + ac_x, y + ac_y, component.order]
                                != 0
                            ):
            ...
            for component in self.color_components.values():
                ...
                for current_mcu in range(mcu_count):
                    ...
                    utils.print_progress(
                        current_mcu + 1, mcu_count, header=component.name.ljust(2)
                    )

                utils.print_progress(
                    current_mcu + 1,
                    mcu_count,
                    header=component.name.ljust(2),
                    done=True,
                )
    ...
    def end_of_image(self, data: bytes) -> None:
        ...
        self.image_array = self.image_array[
            0 : self.image_width, 0 : self.image_height, :
        ]
        """NOTE
        ...
        if self.array_depth == 3:
            self.image_array = converter.YCbCr_to_RGB(self.image_array)
    ...
    def show(self):
        ...
        try:
            ...
            print(
                "The Pillow module needs to be installed in order to display the rendered image."
            )
            print(
                "For installing: https://pillow.readthedocs.io/en/stable/installation.html"
            )
        ...
        my_image = PhotoImage(Image.fromarray(np.swapaxes(self.image_array, 0, 1)))
    ...
    def show2(self):
        ...
        try:
            ...
            print(
                "For installing it: https://pillow.readthedocs.io/en/stable/installation.html"
            )
            return
